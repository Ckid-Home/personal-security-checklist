...
export default component$((props: { sections: Section[] }) => {
  ...
  useOnWindow(
    ...
    $(async () => {
      ...
      completions.value = await Promise.all(props.sections.map((section) => getPercentCompletion(section)));
      // Count of completed items, per section
      done.value = await Promise.all(
        props.sections.map(
          (section) =>
            section.checklist.filter((item) => checked.value[item.point.toLowerCase().replace(/ /g, '-')]).length,
        ),
      );
    }),
  );
  ...
  return (
    <div
      class={[
        styles.container,
        'grid',
        'mx-auto mt-8 px-4 gap-7',
        'xl:px-10 xl:max-w-7xl',
        'transition-all',
        'max-w-6xl w-full',
      ]}
    >
      {props.sections.map((section: Section, index: number) => (
        <a
          ...
          class={[
            'card card-side bg-front shadow-md transition-all px-2',
            `outline-offset-2 outline-${section.color}-400`,
            'hover:outline-3 hover:outline-offset-4',
            `hover:bg-${section.color}-600/15`,
          ]}
        >
          <div class="shrink-0 flex flex-col py-4 h-auto items-stretch justify-evenly">
            ...
            {done.value && done.value[index] ? (
              ...
              <p class={`text-${section.color}-400 pt-2 pb-0 px-0 mx-0 my-0`}>{section.checklist.length} Items</p>
            )}
          </div>
          <div class="card-body grow py-2 pl-4 pr-0">
            ...
            {completions.value && completions.value[index] ? (
              ...
              <span class="absolute right-2 top-2 opacity-30 text-xs">Not yet started</span>
            )}
          </div>
        </a>
      ))}
